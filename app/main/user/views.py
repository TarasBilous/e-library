...
from app.models import User, Log, Permission, INITIAL_BALANCE, DAYS_TO_RETURN
...
def index():
    ...
    income = 0
    for user in users:
        income += INITIAL_BALANCE - user.balance
    return render_template("user.html", users=users, pagination=pagination, title=u"Registered users", income=income)
...
def detail(user_id):
    ...
    return render_template("user_detail.html", user=the_user, logs=logs, pagination=pagination,
                           title=u"User name: " + the_user.name)
    ...
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if avatar_upload_form.validate_on_submit():
            if 'avatar' in request.files:
                ...
                db.session.add(the_user)
                db.session.commit()
                flash(u'Avatar updated', 'success')
                return redirect(url_for('user.detail', user_id=user_id))
        ...
        return render_template('avatar_edit.html', user=the_user, avatar_edit_form=avatar_edit_form,
                               avatar_upload_form=avatar_upload_form, title=u"Update avatar")
    else:
        abort(403)
