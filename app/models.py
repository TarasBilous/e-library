...
class User(UserMixin, db.Model):
    ...
    def borrow_book(self, book):
        ...
        self.balance -= book.collateral_value
    ...
    def return_book(self, log):
        ...
        rent_time = (log.return_timestamp - log.borrow_timestamp).seconds
        if rent_time > DAYS_TO_RETURN:
            correction = PENALTY * rent_time
        else:
            correction = DISCOUNT * (DAYS_TO_RETURN - rent_time)
        log.cost = int(log.book.bookValue) + int(log.book.bookValue) * rent_time + correction
        self.balance -= log.cost - log.book.collateral_value
        db.session.add(log)
...
class Book(db.Model):
    ...
    bookValue = db.Column(db.String(64), default='5')
    thumbnail = db.Column(db.String(128))
    publishedDate = db.Column(db.String(32))
    collateral_value = db.Column(db.Integer)
...
class Log(db.Model):
    ...
    def __init__(self, user, book):
        ...
        self.return_timestamp = datetime.now() + timedelta(days=DAYS_TO_RETURN)
        self.returned = 0
